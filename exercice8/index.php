...
<body>
  ...
  if(!empty($_POST['firstname']) && !empty($_POST['name']) && !empty($_POST['type']) && !empty($_POST['file'])){
    ...
    if(pathinfo($_POST['file'], PATHINFO_EXTENSION) == 'pdf'){
      echo 'Bonjour ' . $_POST['type'] . ' ' .  $_POST['firstname'] . ' ' . $_POST['name'] . ' votre fichier pdf est : ' . $_POST['file'];
    }else{
      echo 'l\'extension de votre fichier est : ' . pathinfo($_POST['file'], PATHINFO_EXTENSION) . ' veuillez le modifier';
    }
  }
  else{
    ?>
    <form action="index.php" method="post">
      ...
      <label>Prénom : <input type="file" name="file"/>value="<?php if (isset($_POST['file'])){echo $_POST['file'];} ?>"</label>
